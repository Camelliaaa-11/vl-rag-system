...
class LabFactExtractor:
    ...
    def _extract_correction(
        ...
        match = re.search(r"不是(.{1,24}?)[，, ]*是(.{1,24})$", text)
        if not match:
            return None
        wrong = self._strip_fact_part(match.group(1))
        correct = self._strip_fact_part(match.group(2))
        ...
        return self._build_entry(
            ...
            user_id=user_id,
            session_id=session_id,
            content=f"用户纠正我：之前识别成{wrong}不对，正确是{correct}。",
        )
