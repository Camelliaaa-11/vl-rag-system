...
class VLMService:
    ...
    def _load_system_prompt(self) -> str:
        path = Config.PROMPTS_DIR / "system_prompt.md"
        try:
            return path.read_text(encoding="utf-8")
        except Exception as exc:
            logger.warning("[VLM] load system_prompt.md failed: %s", exc)
            return "你是艺术导览机器人波普。单次回复最多 100 字。"
    ...
    def _build_user_text(
        ...
        if decision.use_image:
            sections.append(
                "图像指向提示：如果用户的问题和图片有关，请着重识别红色方框内的内容。"
                "默认红色方框内才是用户指到的物品，也就是你需要重点分析的对象。"
            )
    ...
    def generate_visual_summary_sync(
        self,
        image_data: bytes,
        user_text: str = "",
        image_path: str = "",
        ...
        system_prompt = (
            "你是机器人视觉记忆摘要器。请根据图片生成结构化视觉记忆，"
            "只输出 JSON，不要 Markdown，不要解释。"
        )
        user_prompt = (
            "请输出如下 JSON 字段："
            "timestamp, main_content, target_object, scene_summary, confidence, image_path。"
            "main_content 用一句中文概括主要内容，80 字以内；"
            "target_object 写主要物体或未知；confidence 为 0 到 1。"
            f"\n用户当前文字：{user_text or '无'}"
            f"\n图片路径：{image_path or '未知'}"
            f"\n时间戳：{timestamp}"
        )
        messages: List[Dict[str, Any]] = [
            {"role": "system", "content": system_prompt},
        ...
        raw = self._call_provider(messages, use_image=True)
        text = (raw or "").strip()
        if text.startswith("```"):
        ...
        try:
            ...
            parsed = {
                "main_content": self._limit_answer(text) or "视觉摘要生成失败",
                "target_object": "未知",
                "scene_summary": self._limit_answer(text),
                "confidence": 0.5,
            }
        if not isinstance(parsed, dict):
            parsed = {"main_content": str(parsed), "target_object": "未知", "confidence": 0.5}
        parsed.setdefault("timestamp", timestamp)
        parsed.setdefault("image_path", image_path)
        parsed.setdefault("main_content", parsed.get("scene_summary", ""))
        parsed.setdefault("target_object", "未知")
        parsed.setdefault("confidence", 0.8)
        parsed["main_content"] = str(parsed.get("main_content") or "")[:120]
        parsed["scene_summary"] = str(parsed.get("scene_summary") or parsed["main_content"])[:160]
        parsed["target_object"] = str(parsed.get("target_object") or "未知")[:40]
        return parsed
    ...
    def _generate_answer(
        ...
        try:
            ...
            answer = self._limit_answer(self._call_provider(messages, use_image=decision.use_image))
            self.model_status = "ready"
            return {
                "answer": answer,
                "question": question,
